...
def _random_mask(x, batch_axis, axis, min_num, max_num, max_dims, sorted_indices):
    ...
    if isinstance(num, int):
        ...
        _, x = tf.while_loop( cond=lambda i, _: tf.less(i, tf.reduce_max(num)),
                              body=lambda i, x: ( i + 1,
                                                  tf.where( tf.less(i, num),#tf.expand_dims(tf.expand_dims(tf.less(i, num), axis=-1), axis=-1),
...
def specaugment_eval_func(data, network, mask_divisor=5, time_factor=1):
    ...
    num_filters = filter_layer = network.layers["features"].subnetwork.layers["conv_l"].output.shape[-1]
...
def specaug_sorted_second_layer(in_layer, mask_divisor=None):
    """
...
def get_funcs_sorted_second_layer():
    funcs = []
    for k, v in list(globals().items()):
        if k in ["_mask", "_random_mask", "specaugment_eval_func", "sort_filters_by_center_freq"]:
            funcs.append(v)
    return funcs